...
video_test_feat_path = data_path +'testing_data/feat/'
# output_name = 'output_testset.txt'
output_name = sys.argv[2]
default_model_path = 'model-220'
...
def test(model_path=default_model_path):
    test_videos = open(testing_data, 'r').read().split(' ')[:-1]

    ixtoword = pd.Series(np.load('./intra/ixtoword.npy').tolist())
    bias_init_vector = np.load('./intra/bias_init_vector.npy')
